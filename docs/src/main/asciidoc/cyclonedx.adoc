...
NOTE: Embedded dependency SBOMs are also available when running in dev mode (`quarkus dev`).
...
NOTE: The `quarkus.cyclonedx.endpoint.*` configuration options are only available when `quarkus-vertx-http` is present as a direct or transitive dependency of the application, which is the case for most web applications (e.g., those using `quarkus-rest`).

CAUTION: Exposing an SBOM endpoint allows anyone with access to it to perform composition analysis of the application, potentially revealing dependency versions with known vulnerabilities. The endpoint should be restricted to approved consumers only, for example by deploying it on a management interface or by applying appropriate authentication and authorization policies.
