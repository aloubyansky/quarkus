...
1. **Seed propagation**: Scans reachable bytecode to build a method-level call graph and identifies methods that transitively call `ClassLoader.loadClass(String)`, `Class.forName(String)`, or `MethodHandles.Lookup.findClass(String)` using an efficient worklist algorithm. The call graph is preserved and extended incrementally across iterations.
