    ...
    private void collectDependencies(Configuration config, AppModel.Builder appBuilder,
        ...
        for (ResolvedArtifact a : resolvedConfig.getResolvedArtifacts()) {
            ...
            if (LaunchMode.DEVELOPMENT.equals(launchMode)
                ...
                final Project depProject = project.getRootProject().findProject(artifactGa.getArtifactId());
                // ideally, it should be the line below
                //dependency.getArtifact().setPaths(QuarkusGradleUtils.getOutputPaths(depProject));
                // but we need to enhance the dev mode to support multiple output classes dirs for that
                JavaPluginConvention javaConvention = depProject.getConvention().findPlugin(JavaPluginConvention.class);
                if (javaConvention != null) {
