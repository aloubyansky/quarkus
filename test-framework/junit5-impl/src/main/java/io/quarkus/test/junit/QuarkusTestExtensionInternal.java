    ...
    static {
        ...
        if (classLoader instanceof QuarkusClassLoader) {
            ((QuarkusClassLoader) classLoader).addCloseTask(new Runnable() {
                @Override
                public void run() {
                    ScheduledExecutorService h = QuarkusTestExtensionInternal.hangDetectionExecutor;
                    if (h != null) {
                        h.shutdownNow();
                        QuarkusTestExtensionInternal.hangDetectionExecutor = null;
                    }
                }
            });
        }
    ...
    public class ExtensionState extends QuarkusTestExtensionState {
        ...
        protected void doClose() throws IOException {
            ...
            try {
                ...
                clonePattern = null;
                try {
                    if (QuarkusTestExtensionInternal.this.originalCl != null) {
                        setCCL(QuarkusTestExtensionInternal.this.originalCl);
                    }
                    testResourceManager.close();
                } catch (Exception e) {
