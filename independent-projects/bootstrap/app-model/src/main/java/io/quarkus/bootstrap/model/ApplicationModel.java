...
public interface ApplicationModel extends Mappable {
    ...
    default Map<String, Object> asMap(MappableCollectionFactory factory) {
        ...
        map.put(BootstrapConstants.MAPPABLE_DEPENDENCIES,
                Mappable.asMaps(getDependenciesWithAnyFlag(DependencyFlags.DEPLOYMENT_CP | DependencyFlags.COMPILE_ONLY),
                        factory));
        ...
        if (!getReloadableWorkspaceDependencies().isEmpty()) {
            map.put(BootstrapConstants.MAPPABLE_LOCAL_PROJECTS,
                    Mappable.asStringList(getReloadableWorkspaceDependencies(), factory));
        }
        if (!getRemovedResources().isEmpty()) {
            final Map<String, Object> mappedExcludedResources = factory.newMap();
            for (Map.Entry<ArtifactKey, Set<String>> entry : getRemovedResources().entrySet()) {
                mappedExcludedResources.put(entry.getKey().toString(), Mappable.asStringList(entry.getValue(), factory));
            }
