...
public class ExtensionDevModeConfig implements Serializable, Mappable {
    ...
    public Map<String, Object> asMap(MappableCollectionFactory factory) {
        ...
        if (!jvmOptions.isEmpty()) {
            map.put(BootstrapConstants.MAPPABLE_JVM_OPTIONS, Mappable.asMaps(jvmOptions, factory));
        }
        if (!lockJvmOptions.isEmpty()) {
            map.put(BootstrapConstants.MAPPABLE_LOCK_JVM_OPTIONS, Mappable.asStringList(lockJvmOptions, factory));
        }
        return map;
    }
}
