...
public interface ExtensionCapabilities extends Mappable {
    ...
    default Map<String, Object> asMap(MappableCollectionFactory factory) {
        ...
        if (!getProvidesCapabilities().isEmpty()) {
            map.put(BootstrapConstants.MAPPABLE_PROVIDED, Mappable.asStringList(getProvidesCapabilities(), factory));
        }
        if (!getRequiresCapabilities().isEmpty()) {
            map.put(BootstrapConstants.MAPPABLE_REQUIRED, Mappable.asStringList(getRequiresCapabilities(), factory));
        }
        return map;
    }
}
