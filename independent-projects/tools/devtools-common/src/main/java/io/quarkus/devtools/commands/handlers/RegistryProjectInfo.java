...
public class RegistryProjectInfo {
    ...
    public static RegistryProjectInfo fromCatalogWithKeys(ExtensionCatalog registryCatalog,
        ...
        var availableExtensions = new ArrayList<Extension>(extensionKeys.size());
        Collection<ArtifactKey> unavailableExtensions = Set.of();
        for (var key : extensionKeys) {
            var catalogExt = catalogExtensions.get(key);
            if (catalogExt == null) {
                if (unavailableExtensions.isEmpty()) {
                    unavailableExtensions = new HashSet<>();
                }
                unavailableExtensions.add(key);
            } else {
    ...
    public static RegistryProjectInfo fromCatalogWithExtensions(ExtensionCatalog registryCatalog, List<Extension> extensions,
            Collection<ArtifactKey> unavailableExtensions)
            throws QuarkusCommandException {
    ...
    private final Collection<ArtifactKey> unavailableExtensions;
    ...
    public RegistryProjectInfo(ExtensionCatalog primaryPlatformBom, List<ExtensionCatalog> extensionOrigins,
            List<Extension> extensions, Map<?, ?> projectProperties, Collection<ArtifactKey> unavailableExtensions) {
        ...
        this.unavailableExtensions = unavailableExtensions == null ? Set.of() : unavailableExtensions;
    ...
    public boolean isUnavailable(ArtifactKey key) {
        return unavailableExtensions.contains(key);
    }
}
