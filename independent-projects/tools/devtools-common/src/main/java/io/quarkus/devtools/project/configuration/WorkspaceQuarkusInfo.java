...
public class WorkspaceQuarkusInfo {
    ...
    private static void logInfo(ConfiguredProject project, BootstrapMavenContext mavenCtx, MessageWriter log) throws Exception {
        ...
        var extensionCatalog = registryClient.resolveExtensionCatalog();
        log.info("Resolved catalog in " + (System.currentTimeMillis() - startTime));
        log.info("Quarkus application " + quarkusApp.getId());

        var projectExtDeps = getTopExtensionMap(quarkusApp);
    ...
    private static Map<ArtifactKey, ConfiguredArtifact> getTopExtensionMap(ConfiguredModule quarkusApp) {
        final List<ConfiguredArtifact> directExtDeps = quarkusApp.getDirectExtensionDeps();
        final List<ConfiguredArtifact> topTransitiveExtDeps = quarkusApp.getTopTransitiveExtensionDeps();
        final Map<ArtifactKey, ConfiguredArtifact> topExtensions = new HashMap<>(
                directExtDeps.size() + topTransitiveExtDeps.size());
        for (var e : directExtDeps) {
            topExtensions.put(e.getKey(), e);
        }
        for (var e : topTransitiveExtDeps) {
            topExtensions.put(e.getKey(), e);
        }
