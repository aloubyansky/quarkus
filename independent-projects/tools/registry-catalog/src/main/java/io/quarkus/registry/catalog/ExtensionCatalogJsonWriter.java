...
public class ExtensionCatalogJsonWriter {
     ...
     */
    public static JsonObjectBuilder writeExtensionCatalog(ExtensionCatalog catalog) {
        ...
        putOrigins(json, catalog.getDerivedFrom());
     ...
     * @return a JSON object builder
     */
    public static JsonObjectBuilder writePlatformCatalog(PlatformCatalog catalog) {
        JsonObjectBuilder json = Json.object();
    ...
    private static void putOrigins(JsonObjectBuilder json, List<ExtensionOrigin> origins) {
        if (origins == null || origins.isEmpty()) {
            return;
        }
        JsonArrayBuilder arr = Json.array(origins.size());
